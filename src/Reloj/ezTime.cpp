...
#include "ezTime.h"
    ...
    #define err(args...) \
        if (_debug_level >= ERROR) _debug_device->print(args)
    #define errln(args...) \
        if (_debug_level >= ERROR) _debug_device->println(args)
    #define info(args...)    ""
    #define infoln(args...)  ""
    #define debug(args...)   ""
    #define debugln(args...) ""
    ...
    #define errln(args...) \
        if (_debug_level >= ERROR) _debug_device->println(args)
    #define info(args...) \
        if (_debug_level >= INFO) _debug_device->print(args)
    #define infoln(args...) \
        if (_debug_level >= INFO) _debug_device->println(args)
    #define debug(args...)   ""
    #define debugln(args...) ""
    ...
    #define infoln(args...) \
        if (_debug_level >= INFO) _debug_device->println(args)
    #define debug(args...) \
        if (_debug_level >= DEBUG) _debug_device->print(args)
    #define debugln(args...) \
        if (_debug_level >= DEBUG) _debug_device->println(args)
#endif
    ...
    while (strpos < _posix.length())
    {
        ...
        if (c && state == END_MONTH)
        {
            if (c == '.')
            {
                ...
                if (!end_month) end_month = atoi(_posix.c_str() + strpos);
        }
        if (c && state == END_WEEK)
        {
            if (c == '.')
...
String Timezone::dateTime(const String format /* = DEFAULT_TIMEFORMAT */) { return dateTime(TIME_NOW, format); }

String Timezone::dateTime(const time_t t, const String format /* = DEFAULT_TIMEFORMAT */) { return dateTime(t, LOCAL_TIME, format); }
...
{
    return (defaultTZ->setEvent(function, hr, min, sec, day, mnth, yr));
}
